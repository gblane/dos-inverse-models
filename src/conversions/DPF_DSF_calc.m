function [outp] = DPF_DSF_calc(rs, rd, muaLamb, muspLamb, calcmode, fmod, nin, opt2L)
% DPF_DSF_calc Calculate the Differential Pathlength Factor (DPF) or Dual-Slope Factor (DSF).
%
...
%
% Inputs:
%   rs       - Pencil beam source positions in [x, y, z] with direction vector of [0, 0, 1] [mm]
%   rd       - Detector positions in [x, y, z] [mm]
%   muaLamb  - Vector of absorption coefficients [1/mm]
%   muspLamb - Vector of reduced scattering coefficients [1/mm]
%   calcmode - String specifying the coefficient to calculate (e.g., 'DPF_I', 'DSF_Ph') [-]
%   fmod     - (Optional, default=1.40625e8 Hz) Modulation frequency [Hz]
%   nin      - (Optional, default=1.4) Index of refraction inside the medium [-]
%   opt2L    - (Optional) Structure for two-layer calculations [-]
%
% Outputs:
%   outp     - Calculated DPF or DSF value [unitless]
